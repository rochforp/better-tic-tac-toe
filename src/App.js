...
class TicTacToe extends React.Component {
  ...
  aiInit() {
    if (this.props.singlePlayer) {
      ...
      console.log("The AI is Player:  " + aiPlayer);
      this.setState({aiPlayer})
      if (aiPlayer === 1) {
        this.aiMove();
      }
    }
  ...
  render() {
    ...
    const { winner, aiPlayer} = this.state
    let announcement;

    if (this.state.winner) {
      //var msg = this.state.winner > 2 ? 'It\'s Tie!' : `Player ${this.state.winner} wins!`;
      //var msg = this.state.winner !== aiPlayer || this.state.winner !== 3 ? 'It\'s Tie!' : `You just got beat by a robot :)`;
      if (this.state.winner !== aiPlayer && this.state.winner !== 3){
        var msg = 'You Win!! Take your device inside to claim your prize!'
      }
      if (this.state.winner === 3){
        msg = 'It\'s a Tie'
      }
      else if (this.state.winner === aiPlayer) {
        msg = 'You just got beat by a Robot :)'
      }
      console.log("The ai is player: " + aiPlayer)
      console.log("This state winner is: " + this.state.winner)
      
      //if (this.state.winner !== aiPlayer && this.state.winner !== 3) {
      //  msg = 'You Win!! Show this to the cashier claim your prize!'
      //}
      announcement = (
    ...
    return (
      ...
      <div className="grid">
        { grid }
        { announcement }
        <img src={zmart} alt="Logo" className="img-responsive center-block" />
      </div>
...
TicTacToe.propTypes = { width: number, singlePlayer: Boolean };
TicTacToe.defaultProps = { width: 4, singlePlayer: true };
